...
import { AlertCircle, BriefcaseBusiness, CheckCircle2, FileUser, LayoutTemplate, Palette, Plus, Save, Trash2 } from 'lucide-react'
    ...
    <form onSubmit={save} className="space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_25rem]">
        <div className="min-w-0 space-y-5">
          ...
          {section === 'hero' && <HeroEditor hero={content.hero} profile={content.homeProfile} workContext={content.workContext} projects={content.projects} onHeroChange={(hero) => setContent({ ...content, hero })} onProfileChange={(homeProfile) => setContent({ ...content, homeProfile })} onWorkContextChange={(workContext) => setContent({ ...content, workContext })} />}
...
function HeroEditor({ hero, profile, workContext, projects, onHeroChange, onProfileChange, onWorkContextChange }: { hero: ContentData['hero']; profile: ContentData['homeProfile']; workContext: ContentData['workContext']; projects: PortfolioProjectData[]; onHeroChange: (value: ContentData['hero']) => void; onProfileChange: (value: ContentData['homeProfile']) => void; onWorkContextChange: (value: ContentData['workContext']) => void }) {
  const updateLayer = (index: number, patch: Partial<ContentData['workContext']['layers'][number]>) => onWorkContextChange({ ...workContext, layers: workContext.layers.map((layer, itemIndex) => itemIndex === index ? { ...layer, ...patch } : layer) })
  return <div className="space-y-5"><Card><CardHeader><CardTitle>Главный экран</CardTitle><CardDescription>Первое сообщение, которое видит посетитель.</CardDescription></CardHeader><CardContent className="grid gap-5 md:grid-cols-2"><div className="md:col-span-2"><FormField label="Заголовок" htmlFor="hero-title"><Input id="hero-title" value={hero.title} onChange={(event) => onHeroChange({ ...hero, title: event.target.value })} /></FormField></div><div className="md:col-span-2"><FormField label="Подзаголовок" htmlFor="hero-subtitle"><textarea id="hero-subtitle" value={hero.subtitle} onChange={(event) => onHeroChange({ ...hero, subtitle: event.target.value })} rows={4} className="w-full rounded-xl border border-input bg-background px-3 py-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" /></FormField></div><CtaFields prefix="primary" label="Основное действие" value={hero.ctaPrimary} onChange={(ctaPrimary) => onHeroChange({ ...hero, ctaPrimary })} /><CtaFields prefix="secondary" label="Дополнительное действие" value={hero.ctaSecondary} onChange={(ctaSecondary) => onHeroChange({ ...hero, ctaSecondary })} /></CardContent></Card><Card><CardHeader><CardTitle>Рабочий контур</CardTitle><CardDescription>Карточка справа на первом экране. Проект в фокусе выбирается из уже созданных работ.</CardDescription></CardHeader><CardContent className="space-y-5"><div className="grid gap-4 md:grid-cols-2"><FormField label="Название блока" htmlFor="work-context-title"><Input id="work-context-title" value={workContext.title} onChange={(event) => onWorkContextChange({ ...workContext, title: event.target.value })} /></FormField><FormField label="Статус" htmlFor="work-context-status"><Input id="work-context-status" value={workContext.status} onChange={(event) => onWorkContextChange({ ...workContext, status: event.target.value })} /></FormField><FormField label="Подпись проекта" htmlFor="work-context-focus"><Input id="work-context-focus" value={workContext.focusLabel} onChange={(event) => onWorkContextChange({ ...workContext, focusLabel: event.target.value })} /></FormField><FormField label="Проект в фокусе" htmlFor="work-context-project"><select id="work-context-project" value={workContext.featuredProjectId} onChange={(event) => onWorkContextChange({ ...workContext, featuredProjectId: event.target.value })} className="h-10 w-full rounded-xl border border-input bg-background px-3 text-sm"><option value="">Последний проект</option>{projects.map((project) => <option key={project.id} value={project.id}>{project.title || 'Без названия'}</option>)}</select></FormField></div><FormField label="Заключение" htmlFor="work-context-note"><textarea id="work-context-note" value={workContext.note} onChange={(event) => onWorkContextChange({ ...workContext, note: event.target.value })} rows={3} className="w-full rounded-xl border border-input bg-background px-3 py-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" /></FormField><div className="space-y-3"><div className="flex items-center justify-between gap-4"><p className="text-sm font-semibold">Слои компетенций</p><Button type="button" variant="outline" size="sm" onClick={() => onWorkContextChange({ ...workContext, layers: [...workContext.layers, { id: newId(), label: '', value: '' }] })}><Plus className="mr-2 h-4 w-4" />Добавить</Button></div>{workContext.layers.map((layer, index) => <div key={layer.id} className="grid gap-3 rounded-xl border p-3 md:grid-cols-[1fr_1.4fr_auto]"><FormField label="Название" htmlFor={`work-layer-label-${layer.id}`}><Input id={`work-layer-label-${layer.id}`} value={layer.label} onChange={(event) => updateLayer(index, { label: event.target.value })} /></FormField><FormField label="Содержание" htmlFor={`work-layer-value-${layer.id}`}><Input id={`work-layer-value-${layer.id}`} value={layer.value} onChange={(event) => updateLayer(index, { value: event.target.value })} /></FormField><Button type="button" variant="ghost" size="icon" className="self-end" aria-label={`Удалить слой ${index + 1}`} onClick={() => onWorkContextChange({ ...workContext, layers: workContext.layers.filter((_, itemIndex) => itemIndex !== index) })}><Trash2 className="h-4 w-4" /></Button></div>)}</div></CardContent></Card><Card><CardHeader><CardTitle>Обо мне на главной</CardTitle><CardDescription>Короткая презентация и отобранные компетенции. Резюме редактируется отдельно.</CardDescription></CardHeader><CardContent className="space-y-5"><FormField label="Описание" htmlFor="home-profile-about"><textarea id="home-profile-about" value={profile.about} onChange={(event) => onProfileChange({ ...profile, about: event.target.value })} rows={6} className="w-full rounded-xl border border-input bg-background px-3 py-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" /></FormField><StringListEditor label="Компетенции на главной" values={profile.competencies} onChange={(competencies) => onProfileChange({ ...profile, competencies })} placeholder="Например, Next.js и TypeScript" /></CardContent></Card></div>
}
